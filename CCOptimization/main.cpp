...
int GetCCO(int argc, char ** argv){
	...
	{/* a piece of code to compute the stealthy (hyperuniform) packing at unit number density ... */
		double val, chi = 0, L = 10, sigma, phi = 0.1, S0=0.0;
		std::vector<double> param_vtilde;	// parameter used in vtilde functions
		std::vector<std::pair<double,double>> shells; // (K1a, deltaa) per shell; K2a = K1a + deltaa
		DimensionType dim = 2;
		...
		{
			// Convert shells from encoded (Ka, deltaa) units to absolute k-space (k1, k2)
			...
			if (S0 == 0.0)
			{
				...
				// Add constraints for all k in the union of shells

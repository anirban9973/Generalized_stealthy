 ...
 * --------------------------------- */
void g2Computation::Compute(std::function<const MarkedPointPattern(size_t i)> GetConfigsFunction, size_t NumConfig)
{
	omp_set_num_threads(this->num_threads);
	result.clear();
	if (this->single_sided)
	{
		std::cout << "Compute single-sided g2 from " << g2rmax << "\n";
		WeightedIsotropicSingleSidedTwoPairCorrelation(GetConfigsFunction, NumConfig, 
		g2rmax, result, dr);
	}
	else
	{
		std::cout << "Compute g2 from " << g2rmax << "\n";
		WeightedIsotropicTwoPairCorrelation(GetConfigsFunction, NumConfig, 
		g2rmax, result, dr);
	}
}
...
void g2Computation::Write(const std::string OutputPrefix)
{
	...
	if (this->single_sided)
		WriteFunction(result, (OutputPrefix+std::string("_single-sided-s2")).c_str(),fields);
	else
		WriteFunction(result, (OutputPrefix+std::string("_s2")).c_str(),fields);
	
}

void g2Computation::Plot(const std::string OutputPrefix, const std::string & Title)
{
	PlotFunction_Grace(result, OutputPrefix+std::string("_s2"), "r", "g\\s2\\N(r)", Title);
}
...
void g2Computation::ProcessAdditionalOption(const std::string & option, std::istream & input, std::ostream & output)
{
	...
	if (option == "SingleSided")
	{
		output << "SingleSided correlation function.\n";
...
/* ---------------------------------
 *	Implementations of SkComputation
 * --------------------------------- */
void SkComputation::Compute(std::function<const MarkedPointPattern(size_t i)> GetConfigsFunction, size_t NumConfig)
{
	std::cout << "Compute from " << CircularKMin << " to " << LinearKMax <<"\n";
	omp_set_num_threads(this->num_threads);
	result.clear();
	::IsotropicStructureFactor_weighted(GetConfigsFunction, NumConfig, CircularKMax, LinearKMax, result, KPrecision, SampleProbability, average_option, CircularKMin);
}
...
void SkComputation::Write(const std::string OutputPrefix)
{
	...
	WriteFunction(result, (OutputPrefix+std::string("_chif")).c_str(), fields);
}

void SkComputation::Plot(const std::string OutputPrefix, const std::string & Title)
{
	PlotFunction_Grace(result, OutputPrefix+std::string("_chif"), "k", "S(k)", Title);
}
...
void LocalVariance::Compute(std::function<const MarkedPointPattern(size_t i)> GetConfigsFunction, size_t NumConfig)
{
	omp_set_num_threads(this->num_threads);
	this->mean_NR = std::vector<GeometryVector>(1, GeometryVector(4));
	if (this->use_prt_centers){
		MC_LV_Ensemble(GetConfigsFunction, NumConfig, 
		this->result, this-> Rmax, this-> dR, 1, this->mean_NR, true);
		...
		// 	MC_LV_single(c, result_temp, Rmax, dR, centers);
		// 	pd++;
		...
		std::vector<GeometryVector> centers;
		MarkedPointPattern c = GetConfigsFunction(0);
		GetSamplingPoints(c.GetDimension(), this->num_samp_centers, centers, this->rng);

		MC_LV_Ensemble(GetConfigsFunction, NumConfig, 
		this->result, this-> Rmax, this-> dR, centers, this->mean_NR);
	}
}

void LocalVariance::Write(const std::string OutputPrefix)
{
	std::vector<std::string> fields;
	std::string outputname(OutputPrefix);
	fields.emplace_back("y = Local Variance in particle weights at window radius R\n");	
	if (this->use_prt_centers){
		fields.emplace_back("Use particle centers as window centers\n");			
		outputname += std::string("_varf_self");
	}
	else{
		fields.emplace_back("Use random window centers: "+std::to_string(this->num_samp_centers) + "\n");
		outputname += std::string("_varf");
	}
	fields.emplace_back("R\ty(R)\tErr. y");
	//std::cout<<"Done!\n";
	WriteFunction(this->result, outputname.c_str(), fields);

	if (this->mean_NR.size() > 0){
		std::string name = (this->use_prt_centers)? OutputPrefix + std::string("_means_self.txt") : OutputPrefix + std::string("_means.txt");

		std::fstream save(name.c_str(), std::fstream::out);
		if (!save.good()) {
			std::cerr << "Error opening file for writing: " << name << "\n";
			return;
		}
		save << "#y1 = Local average of particle weights at window radius R \n";

		save << "#R\t Re y1\t Im y1\t err Re y1\t err Im y1\n";
		for (int i = 0; i < this->mean_NR.size(); i++) {
			save << std::fixed << std::setprecision(3) 
				 << this->result[i].x[0] <<"\t";  //R
			save << std::setprecision(8) 
			     << this->mean_NR[i].x[0] << "\t" 
			     << this->mean_NR[i].x[1] << "\t" 
			     << this->mean_NR[i].x[2] << "\t" 
			     << this->mean_NR[i].x[3] << "\n";
...
void LocalVariance::Plot(const std::string OutputPrefix, const std::string &Title)
{
	PlotFunction_Grace(this->result, OutputPrefix+std::string("_varf"), "R", "Local Variance of weights", Title);
}

void LocalVariance::ProcessAdditionalOption(const std::string &option, std::istream &input, std::ostream &output)
{
	if (option == "UseParticleCenters"){
		...
		output << "Input the minimum window radius under which weights are not counted.\n";
		input >> this->Rmin; Rmin_local_variance = this->Rmin;
		output << "Minimum window radius is set to " << this->Rmin << "\n";
	}

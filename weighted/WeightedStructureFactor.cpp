...
/** \file WeightedStructureFactor.cpp
 *	\brief	Function implementations for weighted structure factor. */
...
void IsotropicStructureFactor_weighted(std::function<const PeriodicCellList<std::complex<double>>(size_t i)> GetConfigsFunction, size_t NumConfigs, double CircularKMax, double LinearKMax, std::vector<GeometryVector> & Results, double KPrecision, double SampleProbability, size_t option, double CircularKMin)
{
	Results.clear();
	if(!(CircularKMax > 0))
		return;
	...
	if(Verbosity>1){
		std::cout<<"Computing spectral density";
		if (option == 0){
